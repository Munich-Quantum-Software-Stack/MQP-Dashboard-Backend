...
def test_token_creation(active_client) -> None:
    ...
    token_data = {
        "user_token": active_client.user_token,
    ...
    response = active_client.post("/tokens", json=token_data)
...
def test_token_creation_with_too_many_alive(active_client) -> None:
    ...
    assert (
        response.json["status"] == HTTPStatus.FORBIDDEN
...
def test_fetch_all_tokens(active_client) -> None:
    ...
    response = active_client.get("/tokens", json=user_data)
...
def test_revoking_existing_token(active_client) -> None:
    ...
    response = active_client.delete("/tokens", json=token_data)
...
def test_revoking_non_existing_token(active_client) -> None:
    ...
    token_data = {"token_owner": "test_user", "token_name": "test_non_existing_name"}
