...
def is_user_in_admin_group(identity: str) -> bool:
    ...
    if os.getenv("QUANTUM_DB_TESTING") is not None:
        ...
        user = quantum_db.User.get(identity=identity)  # pylint: disable=no-member
        if user is None:
            return False
...
def resolve_user_group():
    """Authenticate an LDAP user and return their first user group."""
...
@jwt_required()
def admin_panel():
    """Return admin access details for a verified superuser or Editor-group member."""
